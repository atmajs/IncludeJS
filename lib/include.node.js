...
if (global.require) {
    ...
    if (name !== 'amd' && name !== 'commonjs') {
        exports.__require.nativeRequire = global.require;
    }
}
...
function path_resolveCurrent() {
    if (document == null) {
        return global.module == null
            ? ''
            : path_win32Normalize(global.module.parent.filename);
    }
...
var RoutesCtor = /** @class */ (function () {
    ...
    RoutesCtor.prototype.register = function (namespace, route, currentInclude) {
        if (typeof route === 'string' && path_1.path_isRelative(route)) {
            var res = currentInclude || include, location = res.location || path_1.path_getDir(res.url || path_1.path_resolveCurrent());
    ...
    RoutesCtor.prototype.resolve = function (namespace, template) {
        var questionMark = template.indexOf('?'), aliasIndex = template.indexOf('::'), alias, path, query = '', route, i, x, length, arr;
        if (aliasIndex !== -1) {
...
exports.Bin = {
    add: function (type, id_, resource) {
        ...
        exports.bin[type][id] = resource;
    },
    find: function (url) {
        var id = path_1.path_isRelative(url)
            ? '/' + url
            : url;
        for (var type_ in exports.bin) {
            var x = exports.Bin.get(type_, id);
            if (x != null) {
                return x;
            }
        }
    ...
    remove: function (url) {
        var resource = exports.Bin.find(url);
        if (resource == null) {
            return;
        }
        for (var type_ in exports.bin) {
            clear(exports.bin[type_], resource);
        }
    ...
    get: function (type, id_) {
        ...
        if (type == null) {
            return exports.Bin.find(id_);
        }
        var id = normalizeId(id_);
        var x = exports.bin[type][id];
        if (x == null && /^https?:\//.test(id) && typeof location !== 'undefined') {
            id = id.replace(location.origin, '');
            x = exports.bin[type][id];
        }
        if (x == null && Config_1.cfg.lockedToFolder) {
            var path = /^file:/.test(id)
                ? path_1.path_getDir(location.href)
                : path_1.path_getDir(location.pathname);
            var sub = path_1.path_combine('/', id.replace(path.toLowerCase(), ''));
            x = exports.bin[type][sub];
        }
        if (x == null && global_1.isBrowser && id[0] === '/') {
            var path = path_1.path_combine(global_2.global.location.origin, id);
            x = exports.bin[type][path];
        }
        return x;
    }
};
...
function bin_removeDelegate(url) {
    ...
    return function () {
        ...
        timeout = setTimeout(function () {
            var triggerFn;
            if (Config_1.cfg.autoreload != null) {
                triggerFn = function (state) {
                    state !== false && Config_1.cfg.autoreload.fileChanged(url, 'include');
                };
            }
...
function bin_remove(path) {
    if (path == null)
        return;
    var type, id, index, res, parents = [];
    for (type in exports.bin) {
        for (id in exports.bin[type]) {
            res = exports.bin[type][id];
...
function bin_clearCache(type, id) {
    var resource = exports.bin[type][id], children = resource.includes;
    delete exports.bin[type][id];
    if (children == null)
        return;
    children.forEach(function (child) {
        var resource = child.resource, type = resource.type, id = resource.url;
        if (id[0] !== '/')
            id = '/' + id;
        delete exports.bin[type][id];
    });
...
exports.PathResolver = {
    ...
    resolveBasic: function (path_, type, parent) {
        ...
        var path = path_1.path_resolveUrl(map(path_), parent);
...
var Resource = /** @class */ (function (_super) {
    ...
    function Resource(type, route, namespace, xpath, parent, id, priority) {
        ...
        if (PathResolver_1.PathResolver.isNpm(_this.url) === false) {
            process(_this);
            return _this;
        }
        if (global_1.isNode === true && PathResolver_1.PathResolver.isNodeNative(_this.url)) {
    ...
    Resource.prototype.contains = function (url, refCache) {
...
function onXHRCompleted(resource, response) {
    ...
    switch (resource.type) {
        ...
        case 'mask':
            if (response) {
                var mask = global_2.global.mask;
                if (mask == null) {
                    mask = global_2.global.require('maskjs');
                }
                mask
                    .Module
...
*/
var isFileProtocol = document && document.location && document.location.protocol === 'file:';
...
;
	...
	if (isObject(_src_node_utils_file) && isObject(module.exports)) {
...
global_1.refs.XMLHttpRequest = /** @class */ (function () {
    ...
    XMLHttpRequest.prototype.send = function () {
        ...
        if (this.url.indexOf('file://') !== -1) {
            this.url = path_1.path_getFile(this.url);
        }
        else if (this.url[0] === '/') {
        ...
        file_1.file_read(this.url, function (err, data) {
            if (err) {
                _this.status = 500;
                data = '';
            }
            ...
            if (err == null && Config_1.cfg.autoreload) {
                file_1.file_watch(_this.url, Bin_1.bin_removeDelegate(_this.url));
            }
        });
    };
...
;
	...
	if (isObject(_src_node_PatchXhr) && isObject(module.exports)) {
...
function internalEval(source, include, isGlobalCtx) {
    ...
    global_1.global.__filename = path_1.path_getFile(include.url);
    global_1.global.__dirname = path_1.path_getDir(global_1.global.__filename);
    ...
    try {
        if (!isGlobalCtx) {
            ...
            module.paths = Module._nodeModulePaths(path_1.path_getDir(filename));
...
;
	...
	if (isObject(_src_node_eval) && isObject(module.exports)) {
...
;
	...
	if (isObject(_src_node_PatchEval) && isObject(module.exports)) {
...
var Bin_1 = _src_Bin;
var Module = require("module");
var npmPath, atmaPath;
Resource_1.Resource.prototype.path_getFile = function () {
    return path_1.path_getFile(this.url);
};
Resource_1.Resource.prototype.path_getDir = function () {
    return path_1.path_getDir(path_1.path_getFile(this.url));
};
Resource_1.Resource.prototype.inject = function () {
    ...
    bundle
        ...
        bundle.on(4, function () {
            var remove = 1;
            var index = array_1.arr_indexOf(current.includes, function (res) {
...
Resource_1.Resource.prototype.instance = function (currentUrl, parent) {
    if (typeof currentUrl === 'string') {
        var old = global_1.global.module, next = new Module(currentUrl, old);
        next.filename = path_1.path_getFile(currentUrl);
        next.paths = Module._nodeModulePaths(path_1.path_getDir(next.filename));
        if (npmPath == null) {
            ...
            if (npmPath == null && process.platform !== 'win32') {
                [
                    ...
                    if (npmPath == null && file_1.fs_exists(path))
                        npmPath = path;
                });
            }
            if (npmPath) {
                if (npmPath.indexOf('node_modules') === -1)
                    npmPath = path_1.path_combine(npmPath, 'node_modules');
                atmaPath = path_1.path_combine(path_1.path_getDir(path_1.path_normalize(process.mainModule.filename)), 'node_modules');
            }
            else {
                npmPath = false;
    ...
    resource = new Resource_1.Resource('js');
    resource.state = 4;
    resource.url = path_1.path_resolveUrl(currentUrl, parent);
    resource.location = path_1.path_getDir(resource.url);
    resource.parent = parent;
    return resource;
};
function inject_process(bundle, index) {
    ...
    var include = bundle.includes[index], resource = include.resource, alias = include.route.alias, source = resource.exports;
    resource.exports = null;
    resource.type = 'js';
...
;
	...
	if (isObject(_src_node_PatchResource) && isObject(module.exports)) {
...
;
	...
	if (isObject(_src_node_export) && isObject(module.exports)) {
